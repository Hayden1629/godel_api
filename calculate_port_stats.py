...
2. For each position, fetch market data (including PE ratio)
3. Calculate portfolio-level statistics including exposure and PE ratios
"""
import time
import requests
...
def get_market_data(client: AccountsTrading, symbol: str) -> Optional[Dict]:
    """
    Get comprehensive market data for a symbol including fundamental data (PE ratio).
...
def print_portfolio_statistics(stats: Dict):
    ...
    print("-"*80)
    print(f"{'Symbol':<10} {'Type':<6} {'Qty':>8} {'Avg $':>10} {'Cur $':>10} {'Value':>12} {'P/L':>25} {'PE':>10}")
    print("-"*80)
    ...
    for pos in stats['positions']:
        ...
        print(f"{symbol:<10} {pos_type:<6} {qty:>7.0f} ${avg_price:>8.2f} ${cur_price:>8.2f} "
              f"${value:>10,.2f} ${pl:>+10,.2f} ({pl_pct:>+6.2f}%) {pe_str:>10}")
